...
df_train = pd.read_csv(output_path +"/df_train.csv",low_memory=False)
df_val = pd.read_csv(output_path +"/df_val.csv",low_memory=False)
df_test = pd.read_csv(output_path +"/df_test.csv",low_memory=False)
...
if training:
    ...
    for epoch in ep_pbar:
        ...
        if BCEloss.item() < best_metric:
            ...
            torch.save(model.state_dict(), output_path +"/epoch_best_model.pt")
...
print()
print("Testing")
model.load_state_dict(torch.load(output_path +"/epoch_best_model.pt"))
model.eval()
...
with torch.no_grad():
    ...
    df_predict.to_csv(output_path +"/prediction_val.csv", index=False)
    print(df_predict)
    ...
    accuracy_df.to_csv(output_path + "/accuracy_persite_val.csv", index=True)
    ...
    df_predict_test.to_csv(output_path +f"/prediction_test.csv", index=False)
